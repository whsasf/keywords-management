      ...
      handleDelete: function (xrow,xtype,xindex){
        ...
        //this.projectLists.splice(xindex,1)
          ...
          if (self.newCategotyFlag){
            ...
            self.axios({
                method: 'delete',
                url: self.baseurl + 'Projects/' + projectId + '/Categories/'  + categoryId,
                withCredentials: 'true',
              })
              .then( res => {
                // console.log(res)
        ...
        if(xtype === 'project'){
          ...
          if (valid) {
              ...
              if ( self.ifChanged(xrow.projectName,xvalue)){
                ...
                let projectId = xrow['_id']['$oid']
                let payLoad = {'projectName':xvalue }
            ...
            if (valid) {
              ...
              if (self.newCategotyFlag){
                ...
                // 同步到 后端
                let projectId = xrow['_id']['$oid']
                let payLoad = {'categoryName':self.normalFormCategory.editCategory}
                self.axios({
                    method: 'post',
                    url: self.baseurl + 'Projects/' + projectId  +'/Categories',
                    withCredentials: 'true',
                    data: payLoad
                  })
                  .then( res => {
              ...
              if ( self.ifChanged(xrow.categories[xindex].categoryName,xvalue)){
                ...
                let categoryId = xrow.categories[xindex]['_id']['$oid']
                let projectId = xrow['_id']['$oid']
                let payLoad = {'categoryName':xvalue}
                self.axios({
                    method: 'patch',
                    url: self.baseurl + 'Projects/' + projectId + '/Categories/' + categoryId,
                    withCredentials: 'true',
